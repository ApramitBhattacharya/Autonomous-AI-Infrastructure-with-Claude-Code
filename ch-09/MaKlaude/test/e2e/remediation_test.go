...
func TestE2E_GatedRemediation(t *testing.T) {
	...
	// --- (e) The record grades the run on the operator's two questions. ---
	//
	// Milestone 6's scoring half over the one live scenario that CONVERGES: everything
	// above reads the cluster and the report, and this reads only what was recorded. It is
	// also where the chaos-window rule meets live facts — see scoring_test.go, which states
	// what that does and does not claim.
	assertRemediationScoresConverged(t, trail, done.proposal)
}
