...
func TestE2E_ChaosDriftsAnApprovedRemediation(t *testing.T) {
	...
	// And the same run graded on the operator's two questions, from the trail rather than
	// from anything asserted above. See scoring_test.go: the assertions above read the
	// report and the wire, which is what MaKlaude DID; this reads what it RECORDED, which
	// is all an incident review six months from now will have.
	assertAbortScoresClean(t, flow.trail, proposal)
}
...
type proxiedExecutorFlow struct {
	...
	// trail is the runner's own audit sink, kept rather than discarded because it is the
	// only thing a verdict may be derived from once the run is over — see scoring_test.go.
	trail *audit.Trail
}
...
func newProxiedExecutorFlow(t *testing.T) *proxiedExecutorFlow {
	...
	return &proxiedExecutorFlow{
		...
		previewer: previewer, gate: gate, sink: sink, runner: runner, trail: trail,
	}
}
